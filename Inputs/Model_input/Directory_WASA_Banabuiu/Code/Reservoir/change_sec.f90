...
IF (dvol_sed(j1,upstream) < 0.) THEN
  elev=erosion_level(j1,upstream)
...
!  elev=min(watelev_sec(j1,upstream)+1.5,watelev_sec(j1,upstream)+((maxelev_sec(j1,upstream)-watelev_sec(j1,upstream))/5.))
  ...
  DO m=1,npoints(j1,upstream)
    if(y_sec(m,j1,upstream)<elev) then
	  dummy5=dummy5+(y_sec(m,j1,upstream)-y_actlay(m,j1,upstream))
	endif
  enddo

  DO m=1,npoints(j1,upstream)
    if(y_sec(m,j1,upstream)<elev .and. dummy5/=0.) then
	  wfactor(m)=(y_sec(m,j1,upstream)-y_actlay(m,j1,upstream))/dummy5
	else
	  wfactor(m)=0.
    endif
  enddo
  ...
  DO m=2,npoints(j1,upstream)-1
    IF (geom(m,j1) == 1)dummy3=dummy3+(((x_sec(m,j1,upstream)-x_sec(m-1,j1,upstream))*wfactor(m)*(1./2.))+ &
							...
							((x_sec(m+1,j1,upstream)-x_sec(m,j1,upstream))*wfactor(m+1)*(1./8.)))
    IF (geom(m,j1) == 2)dummy3=dummy3+((x_sec(m,j1,upstream)-x_sec(m-1,j1,upstream))*wfactor(m)*(3./8.))+ &
							((x_sec(m,j1,upstream)-x_sec(m-1,j1,upstream))*wfactor(m-1)*(1./8.))+ &
							((x_sec(m+1,j1,upstream)-x_sec(m,j1,upstream))*wfactor(m)*(3./8.))+ &
							((x_sec(m+1,j1,upstream)-x_sec(m,j1,upstream))*wfactor(m+1)*(1./8.))
    IF (geom(m,j1) == 3)dummy3=dummy3+((x_sec(m,j1,upstream)-x_sec(m-1,j1,upstream))*wfactor(m)*(3./8.))+ &
							((x_sec(m,j1,upstream)-x_sec(m-1,j1,upstream))*wfactor(m-1)*(1./8.))+ &
							((x_sec(m+1,j1,upstream)-x_sec(m,j1,upstream))*wfactor(m)*(1./2.))
    IF (geom(m,j1) == 4)dummy3=dummy3+((x_sec(m+1,j1,upstream)-x_sec(m-1,j1,upstream))*wfactor(m)*(1./2.))
!if(j1<30)write(*,'(3I4,4F10.2)')j1,m,geom(m,j1),dummy3
  enddo
  
  dummy4=darea_sed(j1,upstream)/dummy3
  DO m=2,npoints(j1,upstream)-1
	y_laststep(m,j1,upstream)=y_sec(m,j1,upstream)
	IF (geom(m,j1) /= 0)y_sec(m,j1,upstream)=y_sec(m,j1,upstream)-dummy4*wfactor(m)	
...
ELSE IF (dvol_sed(j1,upstream) > 0.) THEN
  ...
  DO m=1,npoints(j1,upstream)
    if(y_sec(m,j1,upstream)<watelev_sec(j1,upstream)) then
...
!	  wfactor(m)=1.-((1.-(dummy5))**2.9)
	  wfactor(m)=(watelev_sec(j1,upstream)-y_sec(m,j1,upstream))/(watelev_sec(j1,upstream)-minelev_sec(j1,upstream))
	else
!	  dummy5=0.
	  wfactor(m)=0.
    endif
...
DO g=1,n_sed_class
  if (frac_actlay(g,j1,upstream) /= 0.) then
!	dummy1=diam(g)     
  endif
enddo

...
get_rainy_season <- function(df, years, date_col = 1, skip = 2){
  ...
  #This function will extract the first occurrence of rain in January and
  #the last occurrence of rain in June for the precipitation data in input (df).
  ...
  for (j in seq_len(length(years))){
    for (i in skip:ncol(df)){
      ...
      rainy_season$year[k] <- years[j] # nolint

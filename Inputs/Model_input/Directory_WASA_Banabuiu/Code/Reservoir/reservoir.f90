...
INTEGER :: j,nbrbat1,flag_cav,dummy3,transpo_flag(10000)
...
!    IF (id_subbas_extern(i) == 193) THEN !USE ID 168 FOR CASTANHAO RESERVOIR
    ...
    OPEN(idummy,FILE=pfadp(1:pfadj)//'Reservoir/exttranspo_'//trim(adjustl(subarea))//'_res.dat',IOSTAT=istate,STATUS='old')
...
! Check direct inflow by water transposition into a reservoir
  DO i=1,subasin
    if(transpo_flag(i) == 0) then
      idummy=1000+i
      close(dummy1)
    else if (transpo_flag(i) == 1) then
      idummy=1000+i
      DO id=1,dayyear*nt
        READ (idummy,*)dummy1,extranspo_res(i,id)
        WRITE (*,*)t,id_subbas_extern(i),i,idummy,extranspo_res(i,id)
        if (t == tstop .and. id == dayyear*nt) CLOSE (dummy1)
      ENDDO
    endif
  ENDDO
    ...
    IF (reservoir_balance == 1) THEN
	  ...
	  ELSE
        if (river_transport.eq.1)then ! old routing
	      qinflow(step,upstream)=(qout(step,upstream)+extranspo_res(upstream,step))*(86400./nt)
          qtranspo(step,upstream)=extranspo_res(upstream,step)*(86400./nt)
!write(*,*)upstream,id_subbas_extern(upstream),qout(step,upstream),extranspo_res(upstream,step),qinflow(step,upstream)
	    else ! new routing
!          qinflow(step,upstream)=(r_qout(2,upstream)+qlateral(step,upstream))*(86400./nt) !
          qinflow(step,upstream)=(r_qout(2,upstream)+qlateral(step,upstream)+extranspo_res(upstream,step))*(86400./nt)
!write(*,*)upstream,id_subbas_extern(upstream),r_qout(2,upstream),qlateral(step,upstream),extranspo_res(upstream,step),qinflow(step,upstream)
	    endif
      ENDIF

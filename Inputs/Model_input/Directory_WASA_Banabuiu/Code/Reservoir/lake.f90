...
!*************************************************************************************
REAL :: totallakeinflow,totallakeoutflow,totallakeprec,totallakeevap,totallakevol,totallakerelease !,totallakearea
...
!Ge initialization of output files
  ...
  call open_subdaily_output_lake(f_lake_watbal,'lake_watbal.out','Year, day, hour, totallakeinflow(m**3/timestep), totallakeoutflow(m**3/timestep),lakerelease(m**3/timestep),totallakeprec(m**3/timestep), totallakeevap(m**3/timestep), lakevol(m**3)', subbasin_line=.FALSE.)
...
!Simplified water operation rule. Improve based on NESTRES model (Brasil and Medeiros, 2020)
    DO k=1,5
      help=lakewater(step,muni,k)
      help2=0.
	  if (acud(muni,k)>0.) then
! Variable withdrawal from small reservoirs
!        if (step >= 180 .and. dayyear > step) then
!          help2=help/(dayyear-step+1)
!        else if (step == dayyear) then
!          help2=lakewater(step,muni,k)
!        else if (step < 180) then
!          help2=0.
!        endif
!        lakeex(step,muni,k)=help2
!        lakewater(step,muni,k)=max(0.,help-help2)
!Fixed withdrawal from small reservoirs
        if (step == 180) then
          help2=(help/(dayyear-step+1))*(.85)
        else if (step > 180) then
          help2=min(lakeex(180,muni,k),lakewater(step,muni,k))
        else
          help2=0.
        endif
        lakeex(step,muni,k)=help2
        ...
        lakeex(step,muni,k)=0.
      endif
!    write(*,*)step,dayyear,help,lakewater(step,muni,k),lakeex(step,muni,k)
!    if (step>=180) pause
    ENDDO
...
!  total amount of water storaged in sub-basin (m**3)
    ...
    lakerelease(step,muni)=sum(lakeex(step,muni,1:5))
    ...
    OPEN(11,FILE=pfadn(1:pfadi)//'lake_watbal.out',STATUS='old',  &
	  ...
	  DO d=1,dayyear
	    DO ih=1,nt
          ...
          totallakerelease=0.
		  DO imun=1,subasin
            totallakerelease=totallakerelease+lakerelease(step,imun) !water release from small reservoirs
			...
			ENDDO
		  ENDDO
	      WRITE(11,'(3I6,7f15.3)')t,d,hour,totallakeinflow,totallakeoutflow,totallakerelease,totallakeprec,&
			totallakeevap,totallakevol
		ENDDO
	  ENDDO
    CLOSE(11)
